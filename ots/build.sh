...
# =============================================================================
# 1. Reads config.env (domains, SSH key, coordinates, etc.)
...
# Find block: comment line, path line, permissions, encoding, content + b64 data
pattern = (
...
if match:
    ...
    # Block not found — add before swap section
    new_block = f"""
  {comment}
  - path: {target_path}
    permissions: '0755'
